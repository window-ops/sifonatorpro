...
function setMobileMenu(open) {
  ...
  toggle.setAttribute('aria-label', open ? 'Inchide meniul principal' : 'Deschide meniul principal');
}
...
function nav(page, docId, updateHash) {
  ...
  if (page === 'documentatie') {
    if (docId) { scrollDoc(docId, null, false); } else { setupDocsObserver(); }
  } else if (docId) {
    var t = document.getElementById('sec-' + page + '-' + docId);
...
function setupDocsObserver() {
  ...
  docsObserver = new IntersectionObserver(function(entries) {
    ...
    entries.forEach(function(entry) {
      ...
      setActiveDocLink(id);
      if (!isSyncingHash) { history.replaceState(null, '', '#documentatie:' + id); }
    });
  ...
  sections.forEach(function(section) {
    ...
    if (h2 && !h2.querySelector('.doc-permalink')) {
      ...
      a.setAttribute('aria-label', 'Permalink către aceasta sectiune');
      a.textContent = '#';
      a.onclick = function(e) { e.preventDefault(); scrollDoc(sid, null, true); };
      h2.appendChild(a);
    }
  });
}
